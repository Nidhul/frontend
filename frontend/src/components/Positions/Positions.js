...
const Positions = () => {
  ...
  const fetchData = async (token) => {
    try {
      const response = await axios.get("http://127.0.0.1:8000/angel/angel_positions/",{
        headers:{
...
{positions === null ?  (<p className="no-positions">No Positions Found</p>) : (
  ...
  return position.netqty > 0 ? (
    ...
    <tr className="closed-order" key={index}>
    <td className="closed-order-text">{index + 1}</td>
    <td className="closed-order-text">{position.symbolname}</td>
    <td className="closed-order-text">{position.producttype}</td>
    <td className="closed-order-text">{position.netqty}</td>
    <td className="closed-order-text">{position.avgnetprice}</td>
    <td className="closed-order-text">{position.pnl}</td>
    <td>
      {/* <button
        className="badge-dark-modify"
        onClick={() => handleModify(position)} // Pass the position to handleModify
